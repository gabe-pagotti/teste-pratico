<div>
    ...
    <div>
        <div class="flex items-center justify-end mt-4 mb-4" >
            <a href="{{ route('colaboradores.form') }}" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800" wire:navigate>{{ __('Criar') }}</a>
        </div>
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbdody>
                ...
                <tr wire:key="{{ $colaborador->id }}" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                    ...
                    <td class="px-6 py-4">{{ $colaborador->unidade->nome }}</td>
                    <td class="px-6 py-4">{{ $colaborador->created_at->format("d/m/Y H:i:s") }}</td>
                    <td class="px-6 py-4 flex">
                        <a class="underline dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-gray-800" href="{{ route('colaboradores.show', $colaborador->id) }}" wire:navigate>
                        ...
                        <button type="button" wire:click="delete({{$colaborador->id}})" wire:confirm="Tem certeza que deseja excluir esse colaborador?">
